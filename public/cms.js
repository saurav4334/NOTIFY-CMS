...
// Same-origin by default so it works on any host (localhost, notifybd.com, …).
// Override on a page with: window.NOTIFY_API_BASE = 'https://…/api/v1';
const NOTIFY_API = window.NOTIFY_API_BASE || '/api/v1';
...
function _applySEO(seo) {
  // Server-rendered SEO (a canonical tag is present) always wins — never clobber
  // the page's real title/description/meta with the JS fallback map.
